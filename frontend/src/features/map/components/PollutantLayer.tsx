...
export function PollutantLayer({ featureLayer, map }: Props) {
  ...
  useEffect(() => {
    if (!pollutantData?.data || !map) return;
    ...
    const updatePositions = () => {
      container.innerHTML = '';
      const activeSensors = pollutantData.data.filter(s => {
      ...
      activeSensors.forEach((d: any) => {
        ...
        marker.innerHTML = `
          <div class="pin-wrapper" style="transform: translate(-12px, -24px); transition: all 0.3s ease; color: ${color};">
        ...
        `;
        marker.onmouseenter = () => setHoveredSensor({ ...d, ...pos });
    ...
    if (!document.getElementById(styleId)) {
      ...
      style.id = styleId;
      style.innerHTML = `
        .manual-pin-marker:hover .pin-wrapper {
          transform: translate(-12px, -24px) scale(1.4) translateY(-6px) !important;
          filter: brightness(1.2) drop-shadow(0 4px 8px rgba(0,0,0,0.5)) !important;
          z-index: 10000;
        }
      `;
